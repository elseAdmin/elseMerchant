import 'package:elseMerchant/Contants.dart';
import 'package:elseMerchant/LoadingDialog.dart';
import 'package:elseMerchant/StartUpData.dart';
...
class _OauthManager extends State<OauthManager> {
  ...
  Future<void> _sendCodeToPhoneNumber() async {
    ...
    final PhoneVerificationFailed verificationFailed =
      ...
      setState(() {
        print(
            'Phone number verification failed. Message: ${authException.toString()}');
        _message =
            'Phone number verification failed. Message: ${authException.toString()}';
      });
    };
  ...
  Future _checkForNewUser(
      BuildContext context, CustomUser firebaseAuthUser) async {
      CustomUser firestoreUser = await userProvider.getUserById(firebaseAuthUser.id);
  ...
  void _signInWithPhoneNumber(BuildContext context, String smsCode) async {
    ...
    final CustomUser currentFirebaseUser = await _auth.currentUser();
                          ...
                          child: RaisedButton(
                            onPressed: () async {
                              _signInWithPhoneNumber(
                                  context, _smsCodeController.text);
                            },
                            color: Colors.blueGrey,
                            child: const Text(
                              'Login',
